...
int main()
{
  ...
  assert(s);
  double expected[] = {1.25, 2.0, 3.5};
  int sz = sizeof(expected) / sizeof(double);
  /* Put with no previous value should save the right value. */
  assert(c_datablock_put_double_array_1d(s, "x", "cow", expected, sz) == DBS_SUCCESS);
  double* val = NULL;
  int length;
  assert(c_datablock_get_double_array_1d(s, "x", "cow", &val, &length) == DBS_SUCCESS);
  assert(length == sz);
  ...
  #define big 100
  double buffer[big];
  assert(c_datablock_get_double_array_1d_preallocated(s, "x", "cow", buffer, &length, big) == DBS_SUCCESS);
  #undef big
  /* Get with a too-small buffer should fail, and leave the buffer
     untouched. The returned value of length will say how big the
     buffer needs to be. */
  #define small 1
  double too_small[small];
  too_small[0] = DBL_MIN;
  length = 0;
  assert(c_datablock_get_double_array_1d_preallocated(s, "x", "cow", too_small, &length, small) ==
         DBS_SIZE_INSUFFICIENT);
  #undef small
  assert(too_small[0] == DBL_MIN);
  assert(length == sz);
  /* Put of the same name into a different section should not collide. */
  assert(c_datablock_put_double_array_1d(s, "y", "cow", expected, sz) == DBS_SUCCESS);
  val = NULL;
  assert(c_datablock_get_double_array_1d(s, "y", "cow", &val, &length) == DBS_SUCCESS);
  TEST_ARRAY(length, val, expected);
  free(val);
  /* Second put of the same name with same type should fail, and the
     value should be unaltered. */
  double another_array[] = { 2.5, 3.75, 4.0, 5.5 };
  const int another_sz = sizeof(another_array) / sizeof(double);
  assert(c_datablock_put_double_array_1d(s, "x", "cow", another_array, another_sz) == DBS_NAME_ALREADY_EXISTS);
  val = NULL;
  length = -1;
  assert(c_datablock_get_double_array_1d(s, "x", "cow", &val, &length) == DBS_SUCCESS);
  ...
  double new_expected[] = { 10.0, 20.5, 20.0, 10.25, 20.5, 10.25 };
  const int new_sz = sizeof(new_expected) / sizeof(double);
  assert(c_datablock_replace_double_array_1d(s, "x", "cow", new_expected, new_sz) == DBS_SUCCESS);
  val = NULL;
  length = -1;
  assert(c_datablock_get_double_array_1d(s, "x", "cow", &val, &length) == DBS_SUCCESS);
  ...
  assert(c_datablock_replace_double_array_1d(s, "x", "no such parameter", new_expected, new_sz) == DBS_NAME_NOT_FOUND);
  val = NULL;
  length = -1;
  assert(c_datablock_get_double_array_1d(s, "x", "no such parameter", &val, &length) == DBS_NAME_NOT_FOUND);
  assert(val == NULL);
  assert(length == -1);
