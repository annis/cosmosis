...
int main()
{
  ...
  double complex e_2_3[2][3] = { {1.25 - 0.5 * _Complex_I, 2.0 + 3.5 * _Complex_I, -0.5},
    {3.5 * _Complex_I, -5.0 + 1.0e-6 * _Complex_I, 5.5 }
  };
  /* Array of three elements, each is an array of 2 complexes. */
  double complex e_3_2[3][2] = { {1.25 - 0.5 * _Complex_I, 2.0 + 3.5 * _Complex_I},
    { -0.5, 3.5 * _Complex_I},
   ...
   */
  int sza = sizeof(e_2_3) / sizeof(double complex);
  assert(sza == 2 * 3);
  int szb = sizeof(e_3_2) / sizeof(double complex);
  assert(szb == 2 * 3);
  /* Make sure the two arrays give the same layout in memory */
  double complex* x = &(e_2_3[0][0]);
  double complex* y = &(e_3_2[0][0]);
  ...
  assert(extents[0] == 2);
  assert(extents[1] == 3);
  double complex xx[extents[0]][extents[1]];
  assert(c_datablock_get_complex_array(s, "a", "e23",
                                       (double complex*)xx, ndims, extents) == DBS_SUCCESS);
  for (int i = 0; i != 2; ++i)
    for (int j = 0; j != 3; ++j)
  ...
  assert(c_datablock_put_complex_array(s, "a", "a4d", (double complex*)a4d, 4, a4d_extents)
         == DBS_SUCCESS);
  int out_extents[4];
  assert(c_datablock_get_complex_array_shape(s, "a", "a4d", 4, &out_extents[0])
         == DBS_SUCCESS);
  for (int i = 0; i != 4; ++i) { assert(out_extents[i] == a4d_extents[i]); }
  double complex out4d[out_extents[0]][out_extents[1]][out_extents[2]][out_extents[3]];
